...
`get_censo_escolar()`, …). Aqui o eixo é por **tema e série histórica**:
indicadores únicos compilados de múltiplas fontes ao longo do tempo.
Convenções de nomenclatura, schema de saída e utilidades de cache são
compatíveis, com vistas a uma eventual fusão.

Inspirações de design: [`geobr`](https://github.com/ipeaGIT/geobr)
(família de funções coerente),
...
uma linha por observação, fontes alternativas para o mesmo indicador
como **linhas distintas** (coluna `source`), agregações como nível de
fator explícito (`"total"`), nunca como `NA`. Vocabulários controlados
de indicadores e fontes ficam em
...
| **M2** | [`get_enrollment()`](https://mancano-tales.github.io/educabr/reference/get_enrollment.md) (BR + UF, séries históricas) + 1ª vinheta | ⏳ |
| **M3** | [`get_schooling()`](https://mancano-tales.github.io/educabr/reference/get_schooling.md) com `dimension` + vinheta de desigualdade + `compute_gap()` | ⏳ |
| **M4** | Storage externo (release Zenodo) + utilidades de cache (`set_cache_dir`, `list_cache`, `clear_cache`) | ⏳ |
| **M5** | Dashboard [`run_dashboard()`](https://mancano-tales.github.io/educabr/reference/run_dashboard.md) bilíngue + deploy shinyapps.io | ⏳ |
...
original impuser restrições; consulte
`inst/dict/vocabularies/sources.yaml`). Veja
[`LICENSE.md`](https://mancano-tales.github.io/educabr/LICENSE.md).
